...
# Template for self-correction prompt injected after a failed prior iteration.
# Lives as a markdown file in prompts/ so prompt-engineering edits don't
# require Python source changes (consistent with other prompts in this dir).
_SELF_CORRECTION_TEMPLATE_PATH = (
    Path(__file__).parent.parent / "prompts" / "qa_self_correction.md"
)


def _render_self_correction_prompt(
    error_message: str,
    consecutive_errors: int,
    qa_session: int,
    spec_dir: Path,
) -> str:
    """Render the self-correction prompt with previous-error context."""
    template = Template(_SELF_CORRECTION_TEMPLATE_PATH.read_text(encoding="utf-8"))
    return template.substitute(
        error_message=error_message,
        consecutive_errors=consecutive_errors,
        qa_session=qa_session,
        spec_dir=str(spec_dir),
        attempt_num=consecutive_errors + 1,
    )
...
async def run_qa_agent_session(
    ...
    if previous_error:
        ...
        consecutive = previous_error.get("consecutive_errors", 1)
        prompt += "\n\n" + _render_self_correction_prompt(
            error_message=previous_error.get("error_message", "Unknown error"),
            consecutive_errors=consecutive,
            qa_session=qa_session,
            spec_dir=spec_dir,
        )
        print(
            f"\n⚠️  Retry with self-correction context (attempt {consecutive + 1})"
        )
